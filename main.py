...
# Judul
st.markdown("<h1>🎉 SELAMAT HAPPY BIRTHDAY GEKTU! 🎉</h1>", unsafe_allow_html=True)
...
if st.button("Confirm", key="confirm_button"):
    ...
    left_img = load_image(r"IMG_0253.JPG")
    right_img = load_image(r"IMG_0257.JPG")

    # Menampilkan gambar di kolom
    col_left, col_ticket, col_right = st.columns(3)

    # Mengubah ukuran gambar agar sama dengan gambar treat
    with col_left:
        if left_img is not None:
            st.image(left_img.resize((300, 300)), caption="💃🏻💃🏻Happy Happy", use_column_width=True)
    with col_ticket:
    ...
    st.markdown("""
        <div style='text-align: center; font-family: "Comic Sans MS", cursive, sans-serif; font-size: 20px; color: #FF6347;'>
